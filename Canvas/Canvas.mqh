...
//--- if arc is obviously not within the rays range, don't draw
   ...
   if((fi3<M_PI_2 || fi3>3*M_PI_2) ||  // лу?3 попадает ?1 ил?4 квадрант
      (fi<M_PI_2 || fi>3*M_PI_2)   ||  // лу?4 попадает ?1 ил?4 квадрант
       fi4-fi3>=M_PI)                  // дуга пройде?чере?правую част?эллипс?
     {
      dx=rx;
      dy=0;
      f=(ry_sq<<1)*((dx-1)*dx)+ry_sq+(rx_sq<<1)*(1-ry_sq);
