...
*/
template<typename N> int ArrayCopyRates( N dest_array[], string symbol=NULL, int timeframe=0); 
...
*/
template<typename N> int ArrayCopySeries(N array[], int series_index, string symbol=NULL, int timeframe=0);
...
*/
template<typename N> int ArrayDimension(N array[]);
...
*/
template<typename N> bool ArrayGetAsSeries(N array[]);
...
*/
template<typename N> int ArrayInitialize(N array[], N value);
 ...
 */
template<typename N> bool ArrayIsSeries(	N array[]);
...
*/
template<typename TYPE> int ArrayMaximum(TYPE array[], int count=WHOLE_ARRAY, int start=0);
...
Parameters:
	...
	start	  -  	Initial search index.
*/
template<typename TYPE> int ArrayMinimum(TYPE array[], int count=WHOLE_ARRAY, int start=0);
...
*/
template<typename N> int ArrayRange(	N array[], int range_index);
...
*/
template<typename TYPE> int  ArrayResize(
...
*/
template<typename TYPE> bool  ArraySetAsSeries(
...
template<typename N> int ArraySize(N array[]);
...
*/
template<typename N> int ArraySort(N array[], int count=WHOLE_ARRAY, int start=0, int sort_dir=MODE_ASCEND);
...
*/
template< typename TYPE > int  ArrayCompare(
...
template< typename TYPE > void  ArrayFree(TYPE  array[]);

//The function fills an array with the specified value.
template<typename TYPE> void  ArrayFill(
	TYPE   array[],      // array
...
template< typename TYPE > bool  ArrayIsDynamic(
